...
function cl_get_follow_requests($limit = 10, $offset = false) {
    global $db, $cl, $me;
...
function cl_notify_user($data = array()) {
    ...
    else if(cl_allow_user_notification($data['user_id'], $data['subject']) != true) {
       return false; 
    }
...
function cl_likify_mentions($text = "") {
    $text = preg_replace_callback('/(?:^|\s|,)\B@([a-zA-Z0-9_]{3,32})/is', function($m) {
...
function cl_get_theme_mode() {
    if (not_empty($_COOKIE["dark_mode"])) {
        return "N";
    }
